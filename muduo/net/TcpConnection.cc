...
{
  loop_->assertInLoopThread();//必须在loop thread执行
  ssize_t nwrote = 0;
...
void TcpConnection::shutdownInLoop()
{
  ...
  if (!channel_->isWriting())//当buffer还有数据，并不会关闭连接，而是要等output buffer的数据全部发送出去
  {
    // we are not writing
...
void TcpConnection::connectEstablished() //shared_ptr:1
{
  loop_->assertInLoopThread();
  assert(state_ == kConnecting);
  setState(kConnected);
  channel_->tie(shared_from_this());
  channel_->enableReading();// 将新建连接的Channel对象加入到POLLER轮询管理器/epoll
...
void TcpConnection::handleClose() //shared_ptr:1
{
  ...
  TcpConnectionPtr guardThis(shared_from_this()); //shared_ptr:2
  connectionCallback_(guardThis);//erase in set<TcpConnectionPtr> 
